...
- (instancetype)init {
    if (self = [super init]) {
        NSDictionary<NSString *, NSString *> *fontNames = @{@"Montserrat-ExtraBold": @"ttf", @"DS-Digital": @"ttf", @"Monofur Bold for Powerline": @"ttf"};
...
- (void)registFont:(NSString *)name type:(NSString *)type {
    NSBundle *bundleForClass = [NSBundle bundleForClass:Font.class];
    NSString *path = [bundleForClass pathForResource:name ofType:type];
